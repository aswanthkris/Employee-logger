import axiosInstance from "@/lib/instances";
interface Payload {
  user: {
    token: string;
  };
}
const commonEndPoint = "worklog";

export const userCommonPostRequest = async (
  apiEndPoint: string,
  payload: Payload,
) => {
  const endPoint = commonEndPoint + apiEndPoint;
  const token = payload?.user?.token;
  const headers = {
    "Content-Type": "application/json",
    Authorization: `Bearer ${token}`,
  };
  try {
    const response = await axiosInstance.post(
      endPoint,
      payload,

      {
        headers,
      },
    );
    return response;
  } catch (error) {
    console.log(error);
    // handleCatchError(error);
    return error;
  }
};

export const POST_API_ENDPOINTS = {
  updateProfile: "/updateProfile",
};

//API CALL FUNCTIONS STARTS HERE>>>>>>>>>>>>>>>>>...

//POST APIS
export const updateProfileApi = async (payload: Payload) => {
  //API FOR UPDATING USER PROFILE
  const response = await userCommonPostRequest(
    POST_API_ENDPOINTS.updateProfile,
    payload,
  )
    .then((response) => {
      return response;
    })
    .catch((error) => {
      console.log(error);
      return null;
    });
  return response;
};
